...
export default function PayPalConfirmation({ checkoutId }: { checkoutId: string }) {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      let terminal = false;
      try {
        const response = await fetch(`/api/paypal/status?checkout_id=${encodeURIComponent(checkoutId)}`, { cache: "no-store", signal: controller.signal });
