...
export default function PayPalProgress({ state, documentsReady = false, compact = false, delayed = false }: {
  state: string; documentsReady?: boolean; compact?: boolean; delayed?: boolean;
}) {
  const paid = state === "PAID";
  const current = !paid ? 0 : documentsReady ? 2 : 1;
  const steps = [
    { label: "Confirming payment", detail: "Secure verification with PayPal", icon: LockKeyhole },
