...
import { articles } from "@/content/documents/resources";
import ArticleSections from "@/components/documents/ArticleSections";
type BlogCategory = "All" | "Documents" | "Technical guides" | "Revisions";
type Post = {
  slug: string;
  title: string;
  excerpt: string;
...
const POSTS: readonly Post[] = articles.map((x, i) => ({
  slug: x.id,
  title: x.title,
  excerpt: x.intro,
  category: (["Documents", "Technical guides", "Revisions"] as const)[i],
  readMins: 1,
  dateLabel: "Coverza journal",
  featured: i === 0,
}));
  ...
  return (
    <div
      className={[
        ...
        featured
          ? "bg-[linear-gradient(180deg,rgba(245,242,255,0.72),rgba(255,255,255,0.96))]"
          : "",
      ].join(" ")}
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        {featured ? (
          <>
            <span className="text-[12px] text-slate-400">•</span>
            <span className="text-[12px] font-semibold text-[rgb(108,76,243)]">
              Featured
            </span>
          </>
        ) : null}
      ...
      <div className="mt-6 flex items-center justify-between">
        <Link
          href={`#${post.slug}`}
          className="inline-flex min-h-11 items-center text-sm font-semibold text-slate-950 underline underline-offset-4"
        >
          Read article →
        </Link>
        <div className="text-[12px] text-slate-500">Read below</div>
      </div>
    </div>
  );
}
...
export default function BlogPage() {
  ...
  return (
    ...
    >
      ...
      <section className="pt-2 sm:pt-4 lg:pt-6">
        <div className="max-w-[76rem]">
          ...
          <p className="mt-10 max-w-[54rem] text-[1.02rem] leading-8 text-slate-600 sm:text-[1.14rem]">
            Thoughtful reads on technical documents, clear references and
            keeping the right information to hand. Explore a topic, then read
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
            <div className="min-w-0">
              ...
              <p className="mt-4 max-w-[38rem] text-[1rem] leading-8 text-slate-600">
                Explore what makes technical information useful, how reference
                material differs from instructions and why document versions
                matter.
              </p>
            </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
          ...
          <div className="rounded-[1.8rem] border border-slate-200/80 bg-white/88 p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-600">
              Choose a topic that matches the information you need.
            </p>

            <div className="mt-5 grid gap-3">
              {[
                { title: "Understanding document scope", href: "/get-quote" },
                { title: "Learner resources", href: "/learner" },
                { title: "Van documents", href: "/van" },
            ...
            <div className="mt-6 rounded-[1.2rem] border border-slate-200/80 bg-slate-50/60 px-4 py-4">
              ...
              <div className="mt-1 text-sm leading-6 text-slate-600">
                Existing customers can access previously issued documents using
                their reference.
              </div>
              <div className="mt-4">
                <Link
      ...
      <section className="mt-16">
        <div className="max-w-[56rem]">
          ...
          <h2 className="mt-4 max-w-[14ch] text-3xl font-extrabold leading-[0.95] tracking-[-0.055em] text-slate-950 sm:text-4xl lg:max-w-[13ch] lg:text-[4rem]">
            {cat === "All"
              ? "Preview posts across the library"
              : `Preview posts: ${cat}`}
          </h2>

          <p className="mt-4 max-w-[44rem] text-[1.02rem] leading-8 text-slate-600 sm:text-[1.08rem]">
            Practical explanations, with the full articles available further
            down this page.
          </p>
        </div>
        ...
        <div className="rounded-[2rem] border border-[rgba(108,76,243,0.10)] bg-[linear-gradient(180deg,rgba(245,242,255,0.72),rgba(255,255,255,0.94))] px-6 py-10 shadow-sm sm:px-8 sm:py-12 lg:px-10 lg:py-14">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="heading-unbalanced text-center text-3xl font-extrabold leading-[0.95] tracking-[-0.055em] text-slate-950 sm:text-4xl lg:text-[3.8rem]">
              Ready to start with your vehicle?
            </h2>

            <div className="mx-auto mt-5 max-w-[38rem]">
              <p className="text-center text-[1.02rem] leading-8 text-slate-600 sm:text-[1.08rem]">
                Enter your vehicle details to continue into the existing
                journey, or explore the guides to understand the document
                services.
              </p>
            </div>
      ...
      <ArticleSections items={articles} />
    </PageShell>
  );
}
