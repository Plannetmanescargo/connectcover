...
export default function ContactPage() {
  const SUPPORT_EMAIL = "support@coverza.uk";
  ...
  const [status, setStatus] = useState<
    "idle" | "sending" | "prepared" | "error"
  >("idle");
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [touched, setTouched] = useState<
    Partial<Record<keyof FormState, true>>
  >({});

  function touch(f: keyof FormState) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!canSend) {
      setErrorMsg("Please check the fields above before continuing.");
      return;
    }
    const subject = encodeURIComponent(
      `[Coverza] ${form.topic} — ${form.name}`,
    );
    const body = [
      `Name: ${form.name}`,
      `Reply email: ${form.email}`,
      `Topic: ${form.topic}`,
      `Order reference: ${form.policyRef}`,
      "",
      form.message,
    ].join("\n");
    ...
    >
      ...
      <section className="pt-2 sm:pt-4 lg:pt-6">
        <div className="max-w-[76rem]">
          ...
          <p className="mt-8 max-w-[46rem] text-[1.02rem] leading-8 text-slate-600 sm:text-[1.14rem]">
            Prepare an email enquiry using the form below. For an existing
            order, include the reference and the document version so we can
            understand what you need.
          </p>

          <div className="mt-6 flex flex-wrap gap-x-7 gap-y-3 text-sm font-medium text-slate-700">
            {["Clear enquiries", "Clear next steps", "No hold music"].map(
              (item) => (
            ...
            {status === "prepared" ? (
              ...
              <div className="relative overflow-hidden rounded-[2.25rem] border border-slate-200/70 bg-white shadow-[0_24px_80px_rgba(108,76,243,0.07),0_2px_8px_rgba(15,23,42,0.04)]">
                ...
                <div className="px-8 py-12 text-center sm:px-12">
                  ...
                  <h2 className="mt-5 text-[1.7rem] font-extrabold tracking-[-0.04em] text-slate-950">
                    Your email draft is ready.
                  </h2>
                  <p className="mt-3 text-[0.95rem] leading-[1.85] text-slate-500">
                    Your reply address is{" "}
                    <span className="font-semibold text-slate-900">
                      {form.email}
                    </span>{" "}
                    Review and send the message in your email app. This website
                    has not sent it.
                  </p>
                  ...
                  <div className="px-7 pb-2 pt-8 sm:px-10">
                    ...
                    <div className="mb-8 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5">
                        {[nameDone, emailDone, topicDone, messageDone].map(
                          (done, i) => (
                            <span
                              key={i}
                              className={[
                    ...
                    <ConvoStep
                      ...
                      hint="We'll use this in our reply."
                      done={nameDone}
                    >
                      <input
                        id="name"
                    ...
                    <ConvoStep
                      ...
                      hint="We'll reply here — check junk too."
                      done={emailDone}
                    >
                      <input
                        id="email"
                    ...
                    {(form.topic === "Documents" ||
                      form.topic === "Document access") && (
                      <ConvoStep
                        n={4}
                        label="Order reference?"
                        hint="Add this if your query relates to an existing order."
                        done={form.policyRef.trim().length > 0}
                      >
                        <input
                          id="policyRef"
                    ...
                    >
                      ...
                      {!errors.message && (
                        <p className="mt-2 text-[12px] text-slate-400">
                          Include your device or browser if something looked
                          broken.
                        </p>
                      )}
                    </ConvoStep>
                  ...
                  <div className="border-t border-slate-100 bg-slate-50/50 px-7 py-5 sm:px-10">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <p className="text-[12px] text-slate-400">
                        Opens your email app. Review and send the message there.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid gap-4">
            ...
            <div className="overflow-hidden rounded-[1.75rem] border border-slate-100 bg-white shadow-[0_4px_20px_rgba(108,76,243,0.05)]">
              <div className="divide-y divide-slate-100">
                <a
                  href={`mailto:${SUPPORT_EMAIL}`}
                  className="flex items-start gap-3.5 px-5 py-4 transition hover:bg-slate-50/60"
                >
            ...
            <div className="overflow-hidden rounded-[1.75rem] border border-[rgba(108,76,243,0.10)] bg-[rgba(108,76,243,0.04)] p-5">
              <p className="text-[10.5px] font-bold uppercase tracking-[0.14em] text-[rgb(108,76,243)]/60">
                Faster options
              </p>
              <p className="mt-1 text-[14px] font-extrabold tracking-tight text-slate-950">
                Before you message
              </p>
              <p className="mt-1.5 text-[12.5px] leading-snug text-slate-500">
                Check the delivery message and reference, then tell us what
                needs attention.
              </p>
              <div className="mt-4 grid gap-2">
                {[
                  { label: "Existing documents", href: "/retrieve-policy" },
                  ...
                  <Link
                    key={href}
                    href={href}
                    className="flex items-center justify-between rounded-[1rem] border border-[rgba(108,76,243,0.14)] bg-white px-4 py-3 text-[13.5px] font-semibold text-slate-900 transition hover:border-[rgba(108,76,243,0.30)] hover:shadow-[0_2px_12px_rgba(108,76,243,0.08)]"
                  >
                    {label}
                    <svg
                      width="13"
            ...
            <div className="overflow-hidden rounded-[1.75rem] border border-slate-100 bg-white p-5">
              ...
              <p className="mt-1.5 text-[12.5px] leading-snug text-slate-500">
                Confirm your vehicle details before continuing through the
                existing journey.
              </p>
              <Link
                href="/get-quote"
      ...
      <section className="mt-16 mb-4">
        <div className="relative overflow-hidden rounded-[2rem] bg-[rgb(108,76,243)] px-8 py-10 shadow-[0_20px_56px_rgba(108,76,243,0.22)] sm:px-10 sm:py-12">
          {/* Radial glow */}
          <div
            className="pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full bg-white/[0.07]"
            aria-hidden="true"
          />
          <div
          ...
          <div className="relative mx-auto max-w-[640px] text-center">
            ...
            <p className="mt-3 text-[0.92rem] leading-[1.75] text-white/70">
              Keep your original delivery message and reference to hand.
              Existing customers can use the document retrieval service.
            </p>
