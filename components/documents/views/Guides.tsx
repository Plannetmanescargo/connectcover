...
import { guides as guideArticles } from "@/content/documents/resources";
import ArticleSections from "@/components/documents/ArticleSections";
const guides: readonly Guide[] = guideArticles.map((x, i) => ({
  title: x.title,
  desc: x.intro,
  href: `#${x.id}`,
  label: ["Scope", "Compatibility", "Delivery", "Revisions", "Collection"][i],
  bullets: ["Clear purpose", "Useful references", "Practical next steps"],
  accent: (["blue", "sky", "indigo", "violet", "blue"] as const)[i],
}));
  ...
  return (
    <div className="rounded-[1.45rem] border border-slate-200/80 bg-white/84 p-5">
      ...
      <div className="mt-5">
        <Link
          href={href}
          className={primary ? "btn-primary !text-white" : "btn-ghost"}
        >
          {cta}
        </Link>
      </div>
...
export default function GuidesPage() {
  return (
    ...
    >
      ...
      <section className="pt-2 sm:pt-4 lg:pt-6">
        <div className="max-w-[76rem]">
          ...
          <p className="mt-10 max-w-[54rem] text-[1.02rem] leading-8 text-slate-600 sm:text-[1.14rem]">
            Understand what makes a useful brief, how to check compatibility and
            what to do when your documents arrive. Clear guidance, ready when
            you need it.
          </p>
      ...
      <section className="mt-16">
        <div className="max-w-[56rem]">
          ...
          <p className="mt-4 max-w-[44rem] text-[1.02rem] leading-8 text-slate-600 sm:text-[1.08rem]">
            Read the guidance below to prepare your requirements and understand
            the scope of your document service.
          </p>
        </div>
        ...
        <div className="rounded-[1.9rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(245,242,255,0.88),rgba(255,255,255,0.96))] p-6 shadow-sm sm:p-8 lg:p-10">
          <div className="max-w-[56rem]">
            ...
            <p className="mt-4 max-w-[38rem] text-[1rem] leading-8 text-slate-600">
              Jump straight to the FAQs, existing document retrieval or the help
              centre when you need a specific answer.
            </p>
          </div>
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            <QuickAction
              title="FAQs"
              desc="Browse clear answers on scope, compatibility, delivery and revisions."
              href="/more/faq"
              cta="Browse FAQs"
            />
      ...
      <ArticleSections items={guideArticles} />
    </PageShell>
  );
}
